...
|----------|---------|-------|
| `GET /tokens?uri=<token-URI>` | Fetch a specific token (RMT/IMT/CORT/PSRT) | Returns synthetic fixtures generated by the compiler harness. |
...
| `GET /jwks.json` | JWKS for token signatures | Matches the fixtures served. |
...
### 7.2 Diff Two Runs (optional MVP stretch)
